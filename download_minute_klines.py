...
def fetch_minute_klines(symbol: str, start_date: str, end_date: str = None) -> List[List]:
    """
    Fetch 1-minute K-line data from Binance API
    ...
    print(f"Downloading 1-minute K-line data for {symbol} from {start_date} to {end_dt.strftime('%Y-%m-%d')}...")
    
    while current_start < end_ms:
        # Calculate end time for this batch (1000 minutes max per request)
        ...
        try:
            ...
            # Print progress every 10000 candles to reduce output
            if len(all_data) % 10000 == 0 or len(all_data) == len(data):
                last_date = datetime.datetime.fromtimestamp(last_timestamp / 1000)
                print(f"Downloaded data up to {last_date.strftime('%Y-%m-%d %H:%M:%S')} ({len(all_data)} candles total)...")
...
def save_to_csv(data: List[List], symbol: str, filename: str = None):
    ...
    Args:
        ...
        filename: Output filename (default: {symbol}_minute_klines.csv)
    """
    if not filename:
        filename = f"{symbol}_minute_klines.csv"
...
def main():
    ...
    print(f"Downloading 1-minute K-line data for the last year...")
    ...
    if data:
        ...
        print(f"\nSuccessfully downloaded {len(data)} 1-minute candles for {symbol}")
    else:
