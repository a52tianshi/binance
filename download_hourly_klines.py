...
def fetch_hourly_klines(symbol: str, start_date: str, end_date: str = None) -> List[List]:
    """
    Fetch hourly K-line data from Binance API
    ...
    print(f"Downloading hourly K-line data for {symbol} from {start_date} to {end_dt.strftime('%Y-%m-%d')}...")
    
    while current_start < end_ms:
        # Calculate end time for this batch (1000 hours max per request)
        current_end = min(current_start + (limit - 1) * 60 * 60 * 1000, end_ms)
...
def save_to_csv(data: List[List], symbol: str, filename: str = None):
    ...
    Args:
        ...
        filename: Output filename (default: {symbol}_hourly_klines.csv)
    """
    if not filename:
        filename = f"{symbol}_hourly_klines.csv"
...
def main():
    ...
    # Download data
    data = fetch_hourly_klines(symbol, start_date, end_date)
    
    if data:
        # Save to CSV
        save_to_csv(data, symbol)
        print(f"\nSuccessfully downloaded {len(data)} hourly candles for {symbol}")
    else:
